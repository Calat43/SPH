...
void whole_system(ParticleParams gas_params, ParticleParams dust_params, ProblemParams problem_params)
{
    ...
    double prev_dvel_xg[3 * gamount - 2];
    double prev_gvel_xd[3 * damount - 2];
...
*/
    ...
    for (int frameId = 0; frameId < floor(T / tau); ++frameId)
    {
        ...
        for(int j = 0; j < 3 * gamount - 2; ++j)
        {
            prev_dvel_xg[j] = image_interpolation_value(prev_image_x_g, prev_image_dvelocity, image_dmass, prev_image_drho,
                                                  prev_image_x_d, j, dust_params, problem_params);
        }
        for(int j = 0; j < 3 * damount - 2; ++j)
        {
            prev_gvel_xd[j] = image_interpolation_value(prev_image_x_d, prev_image_gvelocity, image_gmass, prev_image_grho,
                                                  prev_image_x_g, j, gas_params, problem_params);
        }

        for(int j = 0; j < 3 * gamount - 2; ++j)
        {
            next_image_grho[j] = found_next_rho(image_gmass, prev_image_x_g, prev_image_x_g, j, gas_params, problem_params);
            next_image_gvelocity[j] = found_next_gvelocity(prev_image_x_g[j], prev_image_grho[j], prev_image_gvelocity[j], prev_dvel_xg[j], image_gmass,
                                                     prev_image_x_g, prev_image_grho, j, gas_params, problem_params);
        ...
        for(int j = 0; j < 3 * damount - 2; ++j)
        {
            next_image_drho[j] = found_next_rho(image_dmass, prev_image_x_d, prev_image_x_d, j, dust_params, problem_params);
            next_image_dvelocity[j] = found_next_dvelocity(prev_image_drho[j], prev_image_dvelocity[j], prev_gvel_xd[j], problem_params);
